...
export function assertIsSystemData(data: unknown): asserts data is SystemData {
  ...
  if (!('isProduction' in data)) {
    throw new Error('Settings data is missing the "isProduction" field');
  }
  if (typeof data.isProduction !== 'boolean') {
    throw new Error('Settings data "isProduction" field is not a boolean');
  }
  // version
  if (!('version' in data)) {
  ...
  if (typeof data.userRole !== 'number') {
    throw new Error('Settings data "userRole" field is not a number');
  }
  // userRoleId
  ...
  if (typeof data.userRoleId !== 'string') {
    throw new Error('Settings data "userRoleId" field is not a string');
  }
  // isAdmin
  if (!('isAdmin' in data)) {
