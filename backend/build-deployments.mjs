...
async function buildDeployments() {
  ...
  try {
    ...
    await build({
      ...buildConfig,
      entryPoints: { "idev-suitetools-api": resolve("TypeScripts/SuiteTools/idev-suitetools-api.ts") },
    });

    // Build App deployment bundle
    await build({
      ...buildConfig,
      entryPoints: { "idev-suitetools-app": resolve("TypeScripts/SuiteTools/idev-suitetools-app.ts") },
    });
