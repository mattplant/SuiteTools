...
define(["require", "exports", "N/error", "N/log"], function (require, exports, error, log) {
    ...
    class SuiteToolsController {
        ...
        processPostedData() {
            ...
            const success = this.stApp.stLib.stLibNs.stLibNsRecord.updateCustomRecord('customrecord_idev_suitetools_settings', this.stApp.stAppSettings.recordId, updatedSettings);
        ...
        renderScriptsForm() {
            ...
            // set form input option values dynamically
            const versionOptions = this.retrieveOptionValues('SELECT scriptVersion.id, scriptVersion.name FROM scriptVersion ORDER BY name');
            const typeOptions = this.retrieveOptionValues('SELECT scriptType.id, scriptType.name FROM scriptType ORDER BY name');
            const scriptOptions = this.retrieveOptionValues('SELECT id, name FROM script ORDER BY name');
            const ownerOptions = this.retrieveOptionValues("SELECT employee.id, employee.firstname || ' ' || employee.lastname AS name FROM employee WHERE giveaccess = 'T' AND isinactive = 'F' ORDER BY name");
        ...
        renderScriptLogsForm() {
            ...
            if (users) {
                ...
                results = this.stApp.stModel.getScriptLogsViaSearch(rows, levels, user.text, types, scripts, owners, dateOptions, title, detail);
            }
            else {
                if (dateOptions && Number(dateOptions) > 0) {
                    // use SuiteQL API since no specific user requested and dateOptions is less than a day
                    results = this.stApp.stModel.getScriptLogsViaSuiteQL(rows, levels, types, scripts, owners, dateOptions, title, detail);
                }
                else {
                    // use the search API since dateOptions is specified in days
                    results = this.stApp.stModel.getScriptLogsViaSearch(rows, levels, users, types, scripts, owners, dateOptions, title, detail);
                }
            }
            // display the form
         ...
         */
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        retrieveOptionValues(sql, valueIsName = false) {
