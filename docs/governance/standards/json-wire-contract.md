...
`GET_PAYLOAD_VALIDATED_ENDPOINTS` (RESTlet domain Zod, in addition to the envelope) currently includes: `settings`, `user`/`users`, `role`/`roles`, `job`/`jobs`, `file`/`files`, `script`/`scripts`, `scriptLog`/`scriptLogs`, `jobRun`/`jobRuns`, `logins`, `token`/`tokens` (#44), `optionValues` (#47), `integration`/`integrations` (#48). SPA Integrations UI remains scrape-primary; RESTlet validation still guards the API edge.
